---
hide:
  - navigation
  - tags
search:
  exclude: true
---

# #purity

### Keywords by Co - Occurrence

<a href="../../keyword/print"><button class="btn">#print (2)</button></a> <a href="../../keyword/air"><button class="btn">#air (1)</button></a> <a href="../../keyword/buildability"><button class="btn">#buildability (1)</button></a> <a href="../../keyword/component"><button class="btn">#component (1)</button></a> <a href="../../keyword/reef"><button class="btn">#reef (1)</button></a> 

<ol class = "bordered-list" reversed>
  <li><a href="../../../author/berman_ofer"><b>Berman</b> Ofer</a>, <a href="../../../author/zavadsky_andrey"><b>Zavadsky</b> Andrey</a>, <a href="../../../author/drimer_nitai"><b>Drimer</b> Nitai</a>, <a href="../../../author/shashar_nadav"><b>Shashar</b> Nadav</a> et al. <a href="../../../date/2025/08">(2025-08)</a><br><a href="../../../berm_zava_drim_shas.2025.APC3PAR/">A Pure Ceramic 3D Printed Artificial Reef:<br/>Stability and Response</a></li>
  <li><a href="../../../author/maury-ramirez_anibal"><b>Maury-Ramírez</b> Aníbal</a>, <a href="../../../author/rinke_mario"><b>Rinke</b> Mario</a>, <a href="../../../author/blom_johan"><b>Blom</b> Johan</a> <a href="../../../date/2024/09">(2024-09)</a><br><a href="../../../maur_rink_blom.2024.LCESCaAPBECUTP3PaR/">Low-Carbon Embodied, Self-Cleaning, and Air-Purifying Building Envelope Components Using TiO2 Photocatalysis, 3D Printing, and Recycling</a></li>
</ol>
